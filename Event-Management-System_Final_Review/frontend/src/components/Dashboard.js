...
import { Calendar, Users, Award, TrendingUp, Clock, MapPin, Filter, Search, Star, Download, Edit, Trash2, Send, Plus, CheckCircle, DollarSign } from "lucide-react";
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              className="mt-4 sm:mt-0 inline-flex items-center px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl hover:from-purple-700 hover:to-pink-700 transition-all shadow-lg hover:shadow-xl"
            >
              ...
              events.map((ev, index) => (
                ...
                >
                  <div className="p-6">
                    ...
                    {/* Venue Display */}
                    {(ev.venue && typeof ev.venue === 'object' && ev.venue.name) || ev.location ? (
                      <div className="flex items-center text-gray-500 mb-4">
                        <MapPin className="w-4 h-4 mr-2" />
                        <span className="text-sm">
                          {ev.venue && typeof ev.venue === 'object' && ev.venue.name ? ev.venue.name : ev.location}
                        </span>
                      </div>
                    ) : null}
                    ...
                    {ev.isPaid && (
                      <div className="mb-3 p-3 bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-lg flex items-center justify-between">
                        <span className="text-sm font-medium text-gray-700">Event Price:</span>
                        <span className="text-xl font-bold text-orange-600">
                          {ev.currency === 'INR' ? '₹' : ev.currency === 'USD' ? '$' : ev.currency === 'EUR' ? '€' : '£'}{ev.price}
                        </span>
                      </div>
                    )}
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      {me && ev.registeredUsers && ev.registeredUsers.some((u)=> (typeof u === 'object' ? u._id : u) === me.id) ? (
                        ...
                        <button
                          disabled
                          className="w-full bg-red-100 text-red-500 px-4 py-3 rounded-xl cursor-not-allowed font-medium flex items-center justify-center"
                          title="Cannot register for past events"
                        >
                          Event Ended
                        </button>
                      ) : ev.isPaid ? (
                        <button
                          onClick={() => navigate('/payment', { state: { event: ev } })}
                          className="w-full bg-gradient-to-r from-orange-500 to-red-500 text-white px-4 py-3 rounded-xl hover:from-orange-600 hover:to-red-600 transition-all duration-200 font-medium shadow-lg hover:shadow-xl flex items-center justify-center"
                        >
                          <DollarSign className="w-5 h-5 mr-2" />
                          Pay {ev.currency === 'INR' ? '₹' : ev.currency}{ev.price} & Register
                        </button>
                      ) : (
                        <button
                      ...
                      {/* Ticket Button - For registered users (only for upcoming/ongoing events) */}
                      {me && ev.registeredUsers && ev.registeredUsers.some((u)=> (typeof u === 'object' ? u._id : u) === me.id) && 
                       new Date(ev.endAt || ev.startAt || ev.date) >= new Date() && (
                        <button
                          onClick={() => navigate(`/ticket/${ev._id}`)}
                          className="w-full bg-gradient-to-r from-blue-500 to-cyan-500 text-white px-4 py-3 rounded-xl hover:from-blue-600 hover:to-cyan-600 transition-all duration-200 font-medium shadow-lg hover:shadow-xl flex items-center justify-center"
                        >
                      ...
                      {/* Certificate Button - Only show for registered users */}
                      {me && ev.registeredUsers && ev.registeredUsers.some((u)=> (typeof u === 'object' ? u._id : u) === me.id) && (
                        userCertificates[ev._id] ? (
                          <button
                            onClick={() => {
                              if (!ev._id || !me.id) {
                          ...
                          // Only show "Certificate Pending" for past events, nothing for upcoming events
                          new Date(ev.endAt || ev.startAt || ev.date) < new Date() && (
                            <button
                              disabled
                    ...
                    {/* RSVP Section - Only for registered users and not past events */}
                    {me && ev.registeredUsers && ev.registeredUsers.some((u)=> (typeof u === 'object' ? u._id : u) === me.id) && 
                     new Date(ev.startAt || ev.date) >= new Date() && (
                      <div className="mt-4 pt-4 border-t border-gray-100">
                        <p className="text-xs text-gray-500 mb-2 text-center">Will you attend this event?</p>
                        <div className="flex gap-2">
                          <button
