...
import { Calendar, MapPin, Clock, Ticket, Search, Download, ExternalLink } from "lucide-react";
...
export default function UserRegistrations({ token }) {
  ...
  const downloadAllEvents = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/calendar/export-all', {
        headers: { Authorization: `Bearer ${token}` },
      });
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between">
          ...
          {events.length > 0 && (
            <button
              ...
              className="mt-4 sm:mt-0 inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-xl font-semibold hover:from-purple-700 hover:to-blue-700 transition-all shadow-lg hover:shadow-xl"
            >
              ...
              >
                <div className="p-6">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100 flex gap-2">
                    <button
                      onClick={() => downloadICS(ev._id, ev.title)}
                      className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 bg-purple-50 text-purple-700 rounded-lg hover:bg-purple-100 transition-all text-sm font-medium"
                    ...
                    <button
                      onClick={() => addToGoogleCalendar(ev._id)}
                      className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-all text-sm font-medium"
